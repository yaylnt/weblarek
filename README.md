...
Конструктор:
`constructor(products: IProduct[] = [], previewCard: IProduct | null = null)` - принимает массив товаров и выбранную карточку в виде объекта.

Поля класса:
`products:IProduct[]` - массив товаров.
`_previewCard: IProduct | null` - выбранная для просмотра карточка или null, если ещё не выбрана.

Свойства:
`preview: IProduct | null` - геттер и сеттер выбранной карточки товара для подробного просмотра.
...
Конструктор:
`constructor(cartProducts: IProduct[] | null = [])` - принимает массив товаров, которые пользователь добавил в корзину.

Поля класса:
`cartProducts: IProduct[] | null` - массив товаров, которые пользователь добавил в корзину.

Методы класса:
`getCart(): IProduct[] | null` - получение массива товаров в корзине. 
...
Конструктор:
`constructor(data: IBuyer)` - принимает данные покупателя.

Поля класса:
`payment: TPayment` - тип оплаты, возможные варианты: 'card' | 'cash'
`email: string` - электронная почта.
`phone: string` - номер телефона.
`address: string` - адрес доставки.

Методы класса:
`saveData(data: IBuyer): void` - сохранение данных в модели.
...
Методы класса:
`getCatalog(): Promise<IProduct[]>` - получает каталог товаров с сервера, выполняя GET-запрос.
`createOrder(order: IOrder): Promise<IOrderResponse>` - отправляет данные заказа на сервер методом POST.

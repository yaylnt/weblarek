...
Конструктор:
`constructor(container: HTMLElement, events: IEvents)` - конструктор принимает контейнер для карточки, отображаемой внутри каталога, и брокер событий.
...
`isInCart: boolean` - флаг, указывающий, добавлен ли товар в корзину.
...
`set inCart(value: boolean)` - изменяет состояние кнопки покупки в зависимости от наличия товара в корзине.
...
Конструктор:
`constructor(container: HTMLElement, events: IEvents)` - конструктор принимает контейнер для карточки в корзине и брокер событий.
...
`clear(): void` - очищает поля формы, ошибки и переводит кнопку отправки в неактивное состояние.
...
Методы класса:
`clear(): void` - очищает форму и снимает активный класс с кнопок выбора оплаты.
...
| `card:remove` | Пользователь удалил товар из корзины: либо из карточки товара, либо из корзины | `{ id: string }` | `CardPreview.ts`, `CardBasket.ts` |
